...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002482655863').split()]  # Channel id for auto indexing (make sure bot is admin)
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002561270014'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1002588856029'))  # Bin channel id (make sure bot is admin)
DEENDAYAL_MOVIE_UPDATE_CHANNEL = int(environ.get('DEENDAYAL_MOVIE_UPDATE_CHANNEL', '-1002561270014'))  # Notification of those who verify will be sent to your channel
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1002597012426'))  # Premium logs channel id
auth_channel = environ.get('AUTH_CHANNEL', '')  # Channel/Group ID for force sub (make sure bot is admin)
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1002597012426').split()]
support_chat_id = environ.get('SUPPORT_CHAT_ID', '')  # Support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-4737391358')  # Request channel id (make sure bot is admin)
AUTH_CHANNEL = [int(fch) if id_pattern.search(fch) else fch for fch in environ.get('AUTH_CHANNEL', '-1002482655863').split()]
MULTI_FSUB = [int(channel_id) for channel_id in environ.get('MULTI_FSUB', '-1002482655863').split() if re.match(r'^-?\d+$', channel_id)]  # Channel for force sub (make sure bot is admin)
...
# ============================
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://syamgollapudi94:<Y4*XNXSybk7$?BU>@cluster0.yz10u.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
DATABASE_URI2 = environ.get('DATABASE_URI2', "")
DATABASE_NAME = environ.get('DATABASE_NAME', "cluster0")
COLLECTION_NAME = environ.get('COLLECTION_NAME', 'syamgollapudi94')
...
CAPTION_LANGUAGES = ["Telugu", "Hindi", "Bengali", "Tamil", "English", "Bangla","Malayalam", "Kannada", "Marathi", "Punjabi", "Bengoli", "Gujrati", "Korean", "Gujarati", "Spanish", "French", "German", "Chinese", "Arabic", "Portuguese", "Russian", "Japanese", "Odia", "Assamese", "Urdu"]
...
# ============================
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/AnimetelugucreaterdiscussionChat')
...
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'https://t.me/AnimetelugucreaterdiscussionChat')  # Support group link (make sure bot is admin)
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1002588856029')).split()]
